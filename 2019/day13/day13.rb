        ...
        while full_opcode and method(full_opcode) != :halt
            ...
            if opcode_math?(full_opcode)
               ...
               opcode_adjust_relative_base(@program[curr_pos+1], param_mode(full_opcode, 1))
                ...
                input = game.joystick_pos
                opcode_input(input, @program[curr_pos+1], param_mode(full_opcode, 1))
            
            elsif method(full_opcode) == :opcode_output
                outputs << opcode_output(@program[curr_pos+1], param_mode(full_opcode, 1))
                if outputs.length % 3 == 0
                    if outputs[-3]==-1 && outputs[-2]==0
                        game.score = outputs[-1]
                    else
                        game.add_tile(outputs[-3], outputs[-2], outputs[-1])
                    end
                end
                system("clear")
                puts game
...
game = Game.new
program = IntcodeComputer.new(File.read("day13-input.txt"))
program.program[0] = 2
program.run_program(game)
puts game.score
